...
export default function PublicProfile() {
  ...
  return (
    <div className="mx-auto max-w-xl">
      <button onClick={() => navigate(-1)} className="mb-4 flex items-center gap-1 text-slate-400 hover:text-slate-600">
        <ArrowLeft size={18} /> {t('publicProfile.back')}
      </button>

      {loading && <Card className="p-8 text-center text-slate-400">{t('publicProfile.loading')}</Card>}

      {!loading && !profile && <Card className="p-8 text-center text-slate-400">{t('publicProfile.notFound')}</Card>}
      ...
      {!loading && profile && (
        <>
          ...
          <Card className="mb-6 flex flex-col items-center gap-3 p-8 text-center">
            ...
            {profile.bio && <p className="max-w-md text-sm text-slate-600">{profile.bio}</p>}

            <span
              className={clsx(
                'flex items-center gap-1 rounded-full px-4 py-1 text-sm font-bold',
                'bg-glow-100 text-glow-700',
              )}
            >
              <Flame size={16} /> {t('publicProfile.streakDays', { count: profile.streakCurrent })}
            </span>
            <span className="rounded-full bg-brand-50 px-4 py-1 text-xs font-bold text-brand-700">
            ...
            {friendship?.status === 'pending' && (
              <span className="text-sm text-slate-400">{t('publicProfile.requestPending')}</span>
            )}
            {friendship?.status === 'accepted' && (
              <span className="flex items-center gap-1 text-sm font-semibold text-progress-600">
          ...
          <Card className="p-6">
            <h2 className="mb-4 font-bold text-slate-700">{t('publicProfile.achievements')}</h2>
            {profile.achievements.length === 0 ? (
              <p className="text-sm text-slate-400">{t('publicProfile.noAchievements')}</p>
            ) : (
              <div className="grid grid-cols-3 gap-4">
                {profile.achievements.map((a) => (
                  <div key={a.id} className="flex flex-col items-center gap-1 rounded-2xl border-2 border-glow-300 bg-glow-50 p-4 text-center">
