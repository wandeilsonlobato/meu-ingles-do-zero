  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-brand-50 to-white">
      <div className="relative overflow-hidden">
        ...
        <header className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5">
          ...
          <div className="flex items-center gap-4">
            <div className="flex overflow-hidden rounded-full border border-brand-200 text-xs font-bold">
              <button
                onClick={() => setLocale('pt')}
                className={clsx('px-2 py-1', locale === 'pt' ? 'bg-brand-500 text-white' : 'text-brand-600')}
              >
                PT
              </button>
              <button
                onClick={() => setLocale('en')}
                className={clsx('px-2 py-1', locale === 'en' ? 'bg-brand-500 text-white' : 'text-brand-600')}
              >
                EN
              </button>
            </div>
      ...
      <section className="mx-auto max-w-6xl px-6 py-14">
        <h2 className="mb-8 text-center text-2xl font-extrabold text-slate-800">{t('landing.testimonialsTitle')}</h2>
        <div className="grid gap-5 sm:grid-cols-3">
          {TESTIMONIALS.map((item) => (
            <Card key={item.name} className="p-6">
              <p className="mb-4 text-slate-600 italic">{item.text}</p>
              <p className="font-bold text-brand-700">{item.name}</p>
            </Card>
          ))}
        </div>
      </section>
